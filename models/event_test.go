...
func TestEvent_CreateAndList(t *testing.T) {
	...
	require.NoError(t, models.CreateEvent(db, e))

	list, err := models.ListEvents(db, models.EventFilter{})
...
func TestEvent_ListFilterByEventType(t *testing.T) {
	...
	for i := range events {
		require.NoError(t, models.CreateEvent(db, &events[i]))
	}

	// GLOB pattern matching
	list, err := models.ListEvents(db, models.EventFilter{EventType: "gameserver.*"})
	require.NoError(t, err)
	assert.Len(t, list, 2)
	for _, e := range list {
...
func TestEvent_ListFilterByGameserverID(t *testing.T) {
	...
	now := time.Now()
	e1 := &models.Event{ID: "evt-1", EventType: "gameserver.started", GameserverID: "gs-1", Actor: json.RawMessage(`{}`), Data: json.RawMessage(`{}`), CreatedAt: now}
	e2 := &models.Event{ID: "evt-2", EventType: "gameserver.started", GameserverID: "gs-2", Actor: json.RawMessage(`{}`), Data: json.RawMessage(`{}`), CreatedAt: now}
	require.NoError(t, models.CreateEvent(db, e1))
	require.NoError(t, models.CreateEvent(db, e2))

	list, err := models.ListEvents(db, models.EventFilter{GameserverID: "gs-1"})
	require.NoError(t, err)
	assert.Len(t, list, 1)
	assert.Equal(t, "gs-1", list[0].GameserverID)
}
...
func TestEvent_ListWithPagination(t *testing.T) {
	...
	list, err := models.ListEvents(db, models.EventFilter{
		Pagination: models.Pagination{Limit: 3},
	})
	require.NoError(t, err)
	assert.Len(t, list, 3)
}
...
func TestEvent_PruneEvents(t *testing.T) {
	...
	require.NoError(t, models.CreateEvent(db, old))
	...
	require.NoError(t, models.CreateEvent(db, recent))

	pruned, err := models.PruneEvents(db, 30)
	require.NoError(t, err)
