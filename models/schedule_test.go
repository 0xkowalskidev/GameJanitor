...
func TestSchedule_CreateAndGet(t *testing.T) {
	...
	require.NoError(t, models.CreateSchedule(db, s))
	assert.False(t, s.CreatedAt.IsZero())

	got, err := models.GetSchedule(db, "sched-1")
...
func TestSchedule_GetNotFound(t *testing.T) {
	...
	got, err := models.GetSchedule(db, "nonexistent")
	require.NoError(t, err)
	assert.Nil(t, got)
}
...
func TestSchedule_Update(t *testing.T) {
	...
	s.Enabled = false
	s.LastRun = &now
	require.NoError(t, models.UpdateSchedule(db, s))

	got, err := models.GetSchedule(db, "sched-upd")
	require.NoError(t, err)
	require.NotNil(t, got)
...
func TestSchedule_UpdateNotFound(t *testing.T) {
	...
	err := models.UpdateSchedule(db, s)
	require.Error(t, err)
...
func TestSchedule_ListByGameserver(t *testing.T) {
	...
	s1 := &models.Schedule{ID: "sched-a", GameserverID: "gs-sl", Name: "Alpha", Type: "backup", CronExpr: "0 0 * * *", Payload: json.RawMessage(`{}`), Enabled: true}
	s2 := &models.Schedule{ID: "sched-b", GameserverID: "gs-sl", Name: "Bravo", Type: "restart", CronExpr: "0 6 * * *", Payload: json.RawMessage(`{}`), Enabled: true}
	require.NoError(t, models.CreateSchedule(db, s1))
	require.NoError(t, models.CreateSchedule(db, s2))

	list, err := models.ListSchedules(db, "gs-sl")
	require.NoError(t, err)
	assert.Len(t, list, 2)
	// Ordered by name
...
func TestSchedule_ListByGameserver_Empty(t *testing.T) {
	...
	list, err := models.ListSchedules(db, "gs-nonexistent")
	require.NoError(t, err)
	assert.Empty(t, list)
}
...
func TestSchedule_Delete(t *testing.T) {
	...
	s := &models.Schedule{ID: "sched-del", GameserverID: "gs-sd", Name: "Delete Me", Type: "backup", CronExpr: "0 0 * * *", Payload: json.RawMessage(`{}`), Enabled: true}
	require.NoError(t, models.CreateSchedule(db, s))

	require.NoError(t, models.DeleteSchedule(db, "sched-del"))

	got, err := models.GetSchedule(db, "sched-del")
	require.NoError(t, err)
	assert.Nil(t, got)
}
...
func TestSchedule_DeleteNotFound(t *testing.T) {
	...
	err := models.DeleteSchedule(db, "nonexistent")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not found")
}
...
func TestSchedule_DeleteByGameserver(t *testing.T) {
	...
	for _, id := range []string{"sched-1", "sched-2"} {
		s := &models.Schedule{ID: id, GameserverID: "gs-sbd", Name: id, Type: "backup", CronExpr: "0 0 * * *", Payload: json.RawMessage(`{}`), Enabled: true}
		require.NoError(t, models.CreateSchedule(db, s))
	}

	require.NoError(t, models.DeleteSchedulesByGameserver(db, "gs-sbd"))

	list, err := models.ListSchedules(db, "gs-sbd")
	require.NoError(t, err)
	assert.Empty(t, list)
}
...
func TestSchedule_CascadeOnGameserverDelete(t *testing.T) {
	...
	s := &models.Schedule{ID: "sched-cas", GameserverID: "gs-scas", Name: "Cascade Schedule", Type: "backup", CronExpr: "0 0 * * *", Payload: json.RawMessage(`{}`), Enabled: true}
	require.NoError(t, models.CreateSchedule(db, s))

	// Must delete schedules before gameserver (no ON DELETE CASCADE).
	require.NoError(t, models.DeleteSchedulesByGameserver(db, "gs-scas"))
	require.NoError(t, models.DeleteGameserver(db, "gs-scas"))

	got, err := models.GetSchedule(db, "sched-cas")
	require.NoError(t, err)
	assert.Nil(t, got)
}
