...
func ListGameservers(db *sql.DB, filter GameserverFilter) ([]Gameserver, error) {
	query := "SELECT id, name, game_id, ports, env, memory_limit_mb, cpu_limit, auto_start, container_id, volume_name, status, created_at, updated_at FROM gameservers WHERE 1=1"
	var args []any
...
func CreateGameserver(db *sql.DB, gs *Gameserver) error {
	...
	_, err := db.Exec(
		"INSERT INTO gameservers (id, name, game_id, ports, env, memory_limit_mb, cpu_limit, auto_start, container_id, volume_name, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		gs.ID, gs.Name, gs.GameID, gs.Ports, gs.Env, gs.MemoryLimitMB, gs.CPULimit, gs.AutoStart, gs.ContainerID, gs.VolumeName, gs.Status, gs.CreatedAt, gs.UpdatedAt,
	)
	if err != nil {
...
func UpdateGameserver(db *sql.DB, gs *Gameserver) error {
	...
	result, err := db.Exec(
		"UPDATE gameservers SET name = ?, game_id = ?, ports = ?, env = ?, memory_limit_mb = ?, cpu_limit = ?, auto_start = ?, container_id = ?, volume_name = ?, status = ?, updated_at = ? WHERE id = ?",
		gs.Name, gs.GameID, gs.Ports, gs.Env, gs.MemoryLimitMB, gs.CPULimit, gs.AutoStart, gs.ContainerID, gs.VolumeName, gs.Status, gs.UpdatedAt, gs.ID,
	)
	if err != nil {
